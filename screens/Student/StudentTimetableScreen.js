...
import { getchildrenscheduletimetable } from '../../api/authapi'; // Adjust path as needed
...
const StudentTimetableScreen = () => {
    ...
    const formatTimeTo12Hour = (time24) => {
        if (!time24) return 'N/A'; // handle undefined or null
    ...
    useEffect(() => {
        const fetchTimetable = async () => {
            ...
            setLoading(true);
            try {
                const res = await getchildrenscheduletimetable(token);
                const timetableArray = res.data || [];
                // Transform API data to match the display format

                setScheduleData(timetableArray);
            } catch (error) {
    ...
    const hasClass = (day, time) => {
        return scheduleData.find(item => {
            if (!item.startTime) return false; // skip undefined
            const dayMatch = item.day.toLowerCase() === day.name.toLowerCase();
            const timeMatch = formatTimeTo12Hour(item.startTime) === time;
    ...
    const getClassesForDay = (dayName) => {
        return scheduleData
            .filter(item => item.day.toLowerCase() === dayName.toLowerCase() && item.startTime)
            .sort((a, b) => {
                    ...
                    dayClasses.map((classItem, index) => (
                        <View key={index} style={styles.dailyClassCard}>
                            <View style={styles.dailyTimeContainer}>
                                {/* Fixed: Use formatTimeTo12Hour with startTime */}
                                <Text style={styles.dailyStartTime}>
                                    {formatTimeTo12Hour(classItem.startTime).split(' ')[0]}
                                </Text>
                                <Text style={styles.dailyTimePeriod}>
                                    {formatTimeTo12Hour(classItem.startTime).split(' ')[1]}
                                </Text>
                                <Text style={styles.dailyToText}>to</Text>
                                <Text style={styles.dailyEndTime}>
                                    {formatTimeTo12Hour(classItem.endTime)}
                                </Text>
                            </View>
                            <View style={styles.dailyClassInfo}>
                                {/* Fixed: Display subject and teacher name properly */}
                                <Text style={styles.dailySubject}>{classItem.subject || 'No Subject'}</Text>
                                <Text style={styles.dailyTeacher}>{classItem.teacherName || 'No Teacher'}</Text>
                                <Text style={styles.dailyClass}>{classItem.class || classItem.className || 'No Class Info'}</Text>
                            </View>
                        </View>
                    ))
                ) : (
                        ...
                        {classes.map((classItem, index) => (
                            <View key={index} style={styles.listClassCard}>
                                <View style={styles.listTimeContainer}>
                                    {/* Fixed: Use formatTimeTo12Hour with startTime instead of classItem.time */}
                                    <Text style={styles.listTime}>
                                        {formatTimeTo12Hour(classItem.startTime)} - {formatTimeTo12Hour(classItem.endTime)}
                                    </Text>
                                    <Text style={styles.listClass}>{classItem.class || classItem.className || 'No Class Info'}</Text>
                                </View>
                                <View style={styles.listSubjectContainer}>
                                    <Text style={styles.listSubject}>{classItem.subject || 'No Subject'}</Text>
                                    {/* Added: Display teacher name */}
                                    <Text style={styles.listSubject}>{classItem.teacherName || 'No Teacher'}</Text>
                                </View>
                            </View>
                        ))}
                    </View>
                                                ...
                                                <View key={`${day.name}-${time}`} style={[styles.cell, styles.scheduleCell]}>
                                                    {classData && (
                                                        <View style={styles.classContainer}>
                                                            <Text style={styles.classSubject}>{classData.subject || 'Unknown'}</Text>
                                                            <Text style={styles.classTeacher}>{classData.teacherName}</Text>
                                                            <Text style={styles.classTime}>
                                                                {formatTimeTo12Hour(classData.startTime)} - {formatTimeTo12Hour(classData.endTime)}
                                                            </Text>
                                                        </View>
                                                    )}
                                                </View>
